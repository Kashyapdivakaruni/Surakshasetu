...
/**
 * app/ems/video/[caseId]/page.tsx
 *
 * Paramedic's video consultation room.
 *
 * Mirrors the hospital video page structure but uses the EMS case API
 * (/api/ems/case/[caseId]) for patient data and redirects back to
 * /ems/case/[caseId] on leave.
 *
 * Renders as a full-screen overlay (fixed inset-0 z-[100]) over the EMS
 * layout, matching the pattern used by existing modals in this codebase.
 * The existing app/ems/layout.tsx auth guard still protects this page.
 *
 * On mount:
 ...
 *
 * On leave / disconnect:
 *  Redirects back to /ems/case/[caseId]
 */
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6 bg-slate-950">
      <div className="flex flex-col items-center gap-4">
        <div className="w-20 h-20 rounded-full bg-red-900/50 border border-red-700 flex items-center justify-center">
...
export default function EMSVideoPage({ params }: PageProps) {
  const { caseId } = params;
  ...
  return (
    // Full-screen overlay over the EMS layout (sidebar + header hidden beneath)
